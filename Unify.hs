...
swap this forThat here = swap' here
    where swap' (UnboundT this) =       forThat
          swap' (ArrayT cmp t) =        ArrayT cmp $ swap' t
          swap' (RecordT cmp fields) =  RecordT cmp $ zip (map fst fields) (map swap' $ map snd fields)
...
containsUnbound (RecordT cmp fields) =  or (map containsUnbound (map snd fields))
...
parseSyntacticTypeMR (RecordST fields) = let names = map fst fields
                                         ...
                                         in RecordT cmp $ zip names types
...
selfSub self id (RecordT n bindings) = let ids = map fst bindings
                                           types = map snd bindings
                                       in RecordT n $ zip ids $ map (selfSub self id) types
